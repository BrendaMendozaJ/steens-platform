...
class ComprehendModerationService {
  constructor() {
    ...
    this.cyberbullyingPatterns = [
      /\b(fea|gorda|tonta|estupida|inutil|fracasada|perdedora|idiota|imbecil)\b/i,
      /\b(nadie te quiere|matate|suicidate|no sirves|eres basura|eres una)\b/i,
      /\b(callate|cierra la boca|no hables|no opines|shut up)\b/i,
      /(eres fea|estas fea|que fea|muy fea)/i,
      /(no sirves|no vales|no sabes)/i,
      /(eres tonta|estas tonta|que tonta)/i
    ];
  }
  ...
  localPatternAnalysis(text) {
    ...
    offensiveWords.forEach(word => {
      if (lowerText.includes(word)) {
        ...
        results.cyberbullying.confidence = Math.min(results.cyberbullying.confidence + 0.7, 1.0);
      }
    });
  ...
  makeDecision(analysis, originalText) {
    ...
    if (local.cyberbullying.detected && local.cyberbullying.confidence > 0.3) {
      return {
        blocked: true,
        flagged: true,
