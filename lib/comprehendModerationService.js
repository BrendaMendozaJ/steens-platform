...
class ComprehendModerationService {
  ...
  localPatternAnalysis(text) {
    ...
    // Detección ultra específica de preguntas peligrosas
    const dangerousQuestions = [
      'donde vives', 'dónde vives', 'cual es tu numero', 'cuál es tu número',
      'tu telefono', 'tu teléfono', 'cuantos años', 'cuántos años',
      'que edad', 'qué edad', 'manda foto', 'envia foto', 'envía foto',
      'tu direccion', 'tu dirección', 'donde estudias', 'dónde estudias'
    ];
    
    dangerousQuestions.forEach(question => {
      if (lowerText.includes(question)) {
        results.grooming.detected = true;
        results.grooming.patterns.push(`dangerous_question_${question}`);
        results.grooming.confidence = 1.0; // Máxima confianza
      }
    });
    
    // Detectar patrones con signos de interrogación
    if (text.includes('?')) {
      const questionPatterns = [
        /donde\s+vives\s*\?/i,
        /dónde\s+vives\s*\?/i,
        /cuál?\s+es\s+tu\s+número\s*\?/i,
        /cuántos?\s+años\s*\?/i,
        /qué\s+edad\s*\?/i,
        /tu\s+teléfono\s*\?/i,
        /donde\s+estudias\s*\?/i
      ];
      
      questionPatterns.forEach((pattern, index) => {
        if (pattern.test(text)) {
          results.grooming.detected = true;
          results.grooming.patterns.push(`question_pattern_${index}`);
          results.grooming.confidence = 1.0;
        }
      });
    }
